...
async function main() {
  ...
  app.get('/logout', async (req, res) => {
    ...
    if (refreshToken) {
      axios.post('https://apis.roblox.com/oauth/v1/token/revoke', `token=${refreshToken}&client_id=${clientId}&client_secret=${clientSecret}`, {
        headers: {
    ...
    res.clearCookie('userData').clearCookie('accessToken').clearCookie('refreshToken').clearCookie('idToken').redirect('/')
  })

  app.get('/oauth/callback', async (req, res) => {
    const params = client.callbackParams(req)
    const tokenSet = await client.callback(`http://localhost:${port}/oauth/callback`, params, {
