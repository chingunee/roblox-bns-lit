...
contract PrngSystemContract {
    ...
    function getPseudorandomNumberFromArray(uint32[] memory numbers) external returns (uint32) {
        require(numbers.length > 0, "Array must contain at least one number");
        ...
        uint32 index;
        assembly {
            index := mload(add(result, 0x20))
        }
        randNum = numbers[index % numbers.length];
        return randNum;
    }
